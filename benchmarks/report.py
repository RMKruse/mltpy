...
def _interpret(
    ...
    n_mltpy_faster = int(np.sum(values > 1.0))
    ...
    parts = [
        f"Across {len(ratios)} comparable cells, mltpy is on geometric "
        f"mean **{geomean:.2f}× the speed of R mlt**, and is the faster "
        f"backend in {n_mltpy_faster} of {len(ratios)} cells. ",
        ...
        "mltpy's pure-Python ``_auglag.py`` and R `mlt`'s `alabama::auglag` "
        ...
        "holds.  mltpy is now the faster backend in every cell, and the size "
        "of the margin is set by `n`.  At small `n` the per-fit cost is "
        "dominated by the fixed per-evaluation overhead inside each outer "
        "step — the distribution calls and design-matrix assembly that recur "
        "on every likelihood/gradient evaluation.  Replacing the `scipy.stats` "
        "frozen-distribution objects with direct `scipy.special` ufuncs "
        "(`log_ndtr`, `ndtr`, `expit`, `log_expit`, …) and memoising the "
        "assembled Bernstein `(B, dB)` matrices removed the Python/SciPy call "
        "overhead that previously let R lead the n=100–1000 cells, so mltpy "
        "now edges ahead even there.  At large `n` the per-fit cost shifts to "
        "vectorised linear algebra over the Bernstein design matrix — which "
        "mltpy evaluates once per fit and caches (it depends only on `y` and "
        "the basis order, not on the coefficients) — and the margin widens to "
        "several-fold.  The augmented Lagrangian also freezes its penalty ρ "
        "once the constraints are feasible and accepts the alabama-style "
        "feasible-and-stalled convergence point, so fits no longer burn the "
        "full outer-iteration budget on degenerate active sets.  Absolute "
        "per-fit times of a few milliseconds at small `n` mean timing noise "
        "can shift individual cells by 10–30%, so treat small-`n` ratios as "
...
def build_report(
    ...
    parts = [
        "# mltpy vs. R mlt — Runtime Benchmark",
        "",
        ...
        "- mltpy: `mltpy.MLT(order, support).fit(...)`; "
        ...
        "deterministic seed.",
        "- `Speedup` is `R median / Python median`; values > 1 mean mltpy is faster.",
        "",
    ]
